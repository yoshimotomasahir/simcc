...
with col4:
    Z0 = st.number_input("Min Z", min_value=30, max_value=91, value=30, step=1)
with col5:
...
if st.button("Execute Calculation"):
    ...
    for key, val in Eres.items():
        ax.plot(Zs, np.array(val) * 100, label=key)
    ax.set_xlabel("Z")
    ...
    for key, val in Zres.items():
        ax.plot(Zs, 1 / np.array(val), label=key)
    ax.set_xlabel("Z")
    ...
    fig, ax = plt.subplots(figsize=(5, 2))
    for key, val in Zres.items():
        ax.plot(Zs, np.array(val), label=key)
    ax.set_xlabel("Z")
